...
$translation_gallery_show = [
    'en' => [
        'tooltip' => 'Back to gallery',
    ],
    'ru' => [
        'tooltip' => 'Вернуться в галерею',
    ],
    'cs' => [
        'tooltip' => 'Zpět do galerie',
    ],
];
        ...
        <section class="list">
            <div class="list__container">
                <div class="list__head-wrapper">
                    <a href="<?= PATH . get_lang_path() . '/gallery' ?>" title="<?= $translation_gallery_show[$_SESSION['lang']]['tooltip'] ?>">
                    ...
                    <h2 class="list__label"><?= $folder ?></h2>
                </div>
                <ul class="list__items-wrapper">
                    <?php foreach ($images as $image) : ?>
                        <li class="list__item gallery-item ">
                            <a href="<?= './img/gallery/' . $folder . '/' . $image['full'] ?>" style="background-image: url(<?= './img/gallery/' . $folder . '/' .  $image['small'] ?>);" class="list__item-link blurred" data-fslightbox="gallery">
                                <img
                                    class="post__img"
                                    loading="lazy"
                                    src="<?= './img/gallery/' . $folder . '/' .  $image['medium'] ?>"
